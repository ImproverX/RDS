...
## История изменений
### Версия 3.10
- Улучшена работа с реальной клавиатурой -- добавлено подавление дребезга контактов кнопок, теперь задвоение символов стало проявляться гораздо реже.
- Исправлена ошибка работы со вторым КД.
- Добавлена возможность непосредственного запуска ROM-файлов. При задании в командной строке теперь ищется файл с расширением в таком порядке: COM, BAT, ROM, R0M. Отличие запуска ROM- и R0M-файлов от COM в том, что первые запускаются с отключением КД и им не передаются параметры командной строки.

### Версия 3.09
- Ускорена процедура определения и настройки параметров НЖМД.
- Добавлено предложение о форматировании в случае обнаружения ошибки первого сектора нулевого трека квази-диска.
