...
- Добавлена возможность непосредственного запуска ROM-файлов. При задании в командной строке теперь ищется файл с расширением в таком порядке: COM, BAT, ROM. Отличие запуска ROM-файлов от COM в том, что первые запускаются с отключением КД и им не передаются параметры командной строки. 
